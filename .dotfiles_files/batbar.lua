...
local function factory(args)
    ...
    batbar.colors              = args.colors or batbar.colors
    ...
    batbar.bar = wibox.widget {
        forced_height    = 1,
        forced_width     = 59,
        color            = batbar.colors.medium,
        background_color = batbar.colors.background,
        margins          = 1,
        paddings         = 1,
        ticks            = true,
        ticks_size       = 6,
    ...
    batbar.tooltip = awful.tooltip({ objects = { batbar.bar} })

    function batbar.update(callback)
        ...
        -- Assign colors based on perc
        if bat_now.status == "Charging" then
            if bat_now.perc >= 98 then
                batbar.bar:set_color(batbar.colors.full)
            elseif bat_now.perc > 15 then
                batbar.bar:set_color(batbar.colors.medium)
            else
                batbar.bar:set_color(batbar.colors.low)
            end
        else
            if bat_now.perc >= 98 then
                batbar.bar:set_color(batbar.colors.full)
            elseif bat_now.perc > 15 then
                ...
                batbar.bar:set_color(barbar.colors.low)
            end
        end
        batbar.bar:set_value(bat_now.perc / 100)
        ...
        fmt = fmt .. "Time left: %s\n"
        fmt = fmt .. "Watt: %s"
        local text = string.format(fmt, bat_now.status, bat_now.time, bat_now.watt)
        batbar.tooltip:set_text(text)
    ...
    newtimer("batteries", timeout, batbar.update)

    return batbar
end

return setmetatable(batbar, { __call = function(_, ...) return factory(...) end })
